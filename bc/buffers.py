"""Buffers — the single owner of the grounder's tensor allocation.

Phase 2c of the restructure: buffer/tensor allocation moves here so the
algorithm modules stay pure tensor ops and shape derivation lives in one
place. The design is three tiers:

- Tier 2 (here now): :func:`init_states` — the per-forward state buffers
  (``proof_goals`` / ``grounding_body`` / ``accumulated_body`` /
