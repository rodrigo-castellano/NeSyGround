...
if TYPE_CHECKING:                       # annotation only; guided.py imports from us
...
def fill_flat(flat_source: Tensor, check_arg_source_flat: Tensor,
...
def enumerate_dense_single(B: int, K_r: int, Y_r: int, query_subjs: Tensor,
...
def enumerate_dense_cartesian(B, K_r, query_subjs, query_objs, fv_pred_q, fv_bound_q,
...
def enumerate_flat_cartesian(B, K_r, query_subjs, query_objs, fv_pred_q, fv_bound_q,
                             fv_dir_q, fv_valid_q, has_free_q, active_mask, fact_index,
    ...
    (see ``enumerate_dense_cartesian``) then ``nonzero(combined_mask)`` — but
...
def determined_unknown_count(flat_source, ready_now, arg_src_dep, bpreds_dep,
                              nbody, fact_index, M):
...
def enumerate_flat_pruned(B, K_r, query_subjs, query_objs, fv_pred_q, fv_bound_src_q,
                        ...
                        guided: Optional[GuidedBeam] = None):
    """L3 join: incremental free-var expansion with a width BRANCH PRUNER.

    Same row contract as ``enumerate_flat_cartesian`` (``flat_source[T,2+V]``,
    ...
    rows the final ``filter_flat`` would also reject, so the survivor SET
    is identical to the flat cartesian path.

    ``guided`` (GuidedBeam, default None = byte-identical exhaustive): after
    ...
    for fv in range(V):
        ...
        if w_is_capped and width is not None:
            ...
            du = determined_unknown_count(src, ready, asd, bpd, nb, fact_index, M)
...
__all__ = [
    ...
    "fill_flat",
    "enumerate_dense_single", "enumerate_dense_cartesian", "enumerate_flat_cartesian",
    "enumerate_flat_pruned",
]
