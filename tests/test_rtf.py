...
class TestRTFSmoke:
    ...
    def _make_grounder(self):
        """Large enough KB for RTF to work."""
        facts = torch.tensor([
            [1, 1, 2], [1, 2, 3], [1, 3, 4], [1, 4, 5],
            [1, 1, 3], [1, 2, 4], [1, 3, 5],
        ], dtype=torch.long)
        heads = torch.tensor([[2, 6, 7]], dtype=torch.long)  # derived(V0, V1)
        bodies = torch.tensor([[[1, 6, 7]]], dtype=torch.long)  # base(V0, V1)
    ...
    def test_rtf_runs_without_error(self):
        ...
        assert result.body.shape[0] == 1  # batch dimension correct
