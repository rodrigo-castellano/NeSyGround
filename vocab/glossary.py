...
FIELDS: Mapping[str, str] = {
    ...
    "guided_query_topk": "per-query guided beam widths [B] (learned budgets; carried on RunPlan)",
    "guided_capture": "guided state-beam decision-record list (training capture; RunPlan)",
    "guided_query_offset": "chunk offset lifting local b_idx to pre-chunk query index (RunPlan)",
    "guided_sample_tau": "stochastic state-beam temperature (PL sampling; training; RunPlan)",
    "guided_query_depth": "per-query depth gate [B]: query expands only at steps d < gate (RunPlan)",
