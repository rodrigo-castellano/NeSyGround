...
FIELDS: Mapping[str, str] = {
    ...
    "max_goals": "L cap on a RunPlan",
    "max_fact_pairs_body": "fact-pair body cap on a RunPlan",
    "collect_evidence": "emit trees tier (carried on plans/step inputs)",
    "collect_rule_groundings": "emit firings tier (carried on RunPlan)",
    "collect_mode": "RunPlan: terminal collect mode",
    "flat_intermediate": "flat intermediate steps (carried on RunPlan)",
...
KNOBS: Mapping[str, tuple] = {
    ...
    "width": ("sets", "W", 1),
    "u": ("sets", "W_l", 0),
    "flat_intermediate": ("selects", "flat layout for intermediate steps", True),
    "max_groundings_per_rule": ("caps", "Y_r", 32),
    "cartesian_product": ("selects", "all-entities enumeration (ablation)", False),
    "materialization": ("selects", "pbc layout: cartesian | join", "cartesian"),
    ...
    "max_goals": ("caps", "L", "M+(M-1)*D"),
    "max_states": ("caps", "G", 256),
    ...
    "layout": ("selects", "materialization: auto | dense | flat", "auto"),
