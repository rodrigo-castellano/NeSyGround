...
Uses BCGrounder's compiled forward() with per-depth proof tracking via
collected_mask inspection. States that overflow S are lost, so this approach
may miss proofs that the dynamic approach finds.

Also provides ``run_bfs()`` — a lower-level function that takes a pre-built
BCGrounder + queries tensor and returns per-query BFSResult + BatchBFSStats.
This is used by env_analysis as a thin wrapper over grounder BFS.
...
# ======================================================================
# Data classes (used by env_analysis thin wrapper)
# ======================================================================
...
@dataclass
class BatchBFSStats:
    """Aggregate stats from batched BFS."""
...
# ======================================================================
# Low-level BFS using BCGrounder
# ======================================================================
...
def run_bfs(
    ...
    """Run BFS using BCGrounder, tracking per-depth proofs.

    Manually runs init_states → step loop → filter_terminal, inspecting
    collected_mask between depths to determine when each query is first proved.
    The grounder's step_hook (if set) fires between depths for filtering.

    Args:
        grounder: BCGrounder configured with desired depth, resolution, filter.
        queries_idx: [N, 3] query tensors on device.
        batch_size: Queries per batch (B dimension).
        excluded_queries: Optional [N, 1, 3] excluded queries for cycle prevention.

    Returns:
        depths: [N] tensor of proof depths (-1 if not proved).
        results: List[BFSResult] per query.
        stats: BatchBFSStats (aggregate, some fields empty for static BFS).
    """
    ...
    for batch_start in range(0, N, batch_size):
        ...
        states = grounder.init_states(batch_queries, query_mask, **init_kw)

        for d in range(grounder.depth):
            states = grounder.step(states, d)

            # Step hook (KGE filtering between depths)
            ...
            # Track newly proved queries (any collected grounding → proved)
            has_grounding = states["collected_mask"].any(dim=1)  # [B]
            newly_proved = has_grounding & (batch_depths < 0)
            batch_depths[newly_proved] = d + 1
    ...
    stats = BatchBFSStats(
        avg_max_frontier=0.0,
    )

    n_proven = total_proven
    print(f"  BFS complete: {n_proven}/{N} proved in {elapsed:.1f}s")

    return depths, results, stats


# ======================================================================
# High-level CLI: generate depth files from dataset
# ======================================================================
...
def generate_depths_static(
    ...
    # Create grounder if not provided
    if grounder is None:
        kb = dataset.make_kb()
        grounder = BCGrounder(
            kb,
    ...
    print(f"Grounder: {grounder_type}, K={grounder.K}, S={grounder.S}, G={max_goals}")
    print(f"Max depth: {max_depth}, Batch size: {batch_size}")
    print(f"Device: {dev}, Compile: {compile_mode}")
    print(f"{'='*60}\n")

    # Use run_bfs for the actual BFS
    depths, results, stats = run_bfs(grounder, queries_idx, batch_size)
    t_total = time.time()
